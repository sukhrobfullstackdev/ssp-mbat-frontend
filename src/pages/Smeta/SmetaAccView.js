...
import QuartsByMonthView from "../../components/QuartsByMonthView";
...
const GETSMETA_URL = '/smeta/loadSmetaById';

const SmetaAccView = ({setTitleNav}) => {
    ...
    const quartsRef = useRef();

    /*useEffect(() => {
        setTitleNav('Сметани тахрирлаш');
    }, []);*/
    ...
    const monthsView = [ 'jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec' ]
    ...
    const getSmetaData = async (dataTab) => {
        setLoading(true);

        const { data } = await axios.get(GETSMETA_URL,
            {
                params: {
                    id: smetaIddoc
                },
        ...
        const tt = data.smetapoints.map((elem) => {
            return {
                month: monthsView[elem.month-1],
                sumpay: elem.sumPay,
                expense: elem.expense
            }
        })

        console.log(tt,'databb')
        setLoading(false);
        return
        tt.forEach(item => {

            let oo = dataTab.filter( (elem) => elem.code === item.expense )

            oo.forEach(async (elem) => {
                let expIndex = dataTab.findIndex( (item) => item.code === elem.code );
                if (expIndex === -1 ) return
                await handleRowSelectionView(elem, expIndex)
            })
            console.error(item)
            quartsRef.current.handleSumView(item);
    ...
    const handleRowSelectionView = (record, rowIndex) => {
        console.log(record, 'record')
        console.log(rowIndex, 'rowIndex')
        console.log([record.id], '[record.id]')
        console.log(allExpObj, 'allExpObj')
...
//        console.log(rowIndex, 'rowIndex')
    ...
    const handleChangeQuart = ({allQuartSum, ...obj} ) => {
        ...
        console.log(obj.expCode, 'console.log(obj.expCode)')

        console.log(obj.newObjMonth, 'console.log(newObjMonth)')
        console.log(obj.quartsData, 'console.log(obj.quartsData)')
        console.log(allExpObj, 'allExpObj')
        ...
        if (selectedRow !== -1) handleCellUpdate(selectedRow, 'sumpayexp', allQuartSum)
        else {
            //for VIEW and Edit
                let getRowInd = stateTab.findIndex((elem) => elem.code === obj.expCode)
                handleCellUpdate(getRowInd, 'sumpayexp', allQuartSum)
            }
            ...
            <Layout style={{height:"100%", overflow:"hidden"}}>
                ...
                <div>
                    ...
                    <Table columns={columnsExpSmeta}
                           ...
                           {  return  expCodeState &&
                               <QuartsByMonth expCode={expCodeState}
