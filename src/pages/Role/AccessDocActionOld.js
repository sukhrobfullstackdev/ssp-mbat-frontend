...
const AccessDocAction = (props) => {
    ...
    const [expandedKeys, setExpandedKeys] = useState(['0-1-0-0', '0-2-0-0', '1-3-0-0', '1-4-0-0', '0-5-0-0']);
    ...
    const handleSave = async (selectedKeysValue, info) => {
        ...
        let submitStr = [],
            isAction = checkedKeys.filter(elem => elem.split('-')[3] !== "0" )
        console.log('isAction', isAction);
        isAction.forEach(elem => {
            let doc_type = elem.split('-')[1] // tip dokumenta
            let action = elem.split('-')[3] //deystvie
